...
Handles mode changes, timing, and interactions.
"""

from typing import List, Optional
from .ghost import Ghost, Blinky, Pinky, Inky, Clyde, GhostMode
from .constants import GHOST_VULNERABLE_DURATION
...
class GhostManager:
    ...
    def __init__(self, maze):
        ...
        # Find ghost house position (look for 'G' in maze)
        ghost_house_x, ghost_house_y = self._find_ghost_house(maze)
        
        # Create the four ghosts
        self.ghosts: List[Ghost] = [
            Blinky(ghost_house_x - 1, ghost_house_y),
            Pinky(ghost_house_x, ghost_house_y),
            Inky(ghost_house_x + 1, ghost_house_y),
            Clyde(ghost_house_x, ghost_house_y + 1)
        ]
    ...
    def _find_ghost_house(self, maze) -> tuple:
        """Find the ghost house position in the maze."""
    ...
    def check_collision_with_pacman(self, pacman) -> Optional[Ghost]:
        ...
        Returns:
            Ghost that collides, or None if no collision
        """
    ...
    def get_ghost_positions(self) -> List[tuple]:
    ...
    def eat_ghost(self, ghost: Ghost) -> int:
        ...
        if ghost.is_vulnerable():
            ...
            # Score increases for each ghost eaten in sequence
            vulnerable_count = len(self.get_vulnerable_ghosts())
            return 200 * (2 ** (4 - vulnerable_count))
